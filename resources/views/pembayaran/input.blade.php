<div class="form-group form-textinput" id="form_{{ $item['name'] }}">
        ...
        <div class="form-radio">
            @foreach ($item['value'] as $key => $val)
                <div class="radio radiofill radio-inline">
                    <label>
                        <input type="radio" name="{{ $item['name'] }}" readonly value="{{ $val }}"
    ...
    @if ($item['input'] == 'datetimepicker')
        <input type="text" id="{{ $item['name'] }}" name="{{ $item['name'] }}" readonly
            @if ($store == 'update') value="{{ $data[$item['name']] }}" @else value="{{ old($item['name']) }}" @endif
            class="form-control {{ $errors->has($item['name']) ? 'is-invalid' : '' }}">
    @endif
    @if ($item['input'] == 'date')
        ...
        $(function() {
            @if (isset($item['input']))
                @if ($item['input'] == 'combo')
                    ...
                    $("#cmb{{ $item['name'] }}").select2("readonly", true);
                @endif
            @endif
            @if (isset($item['input']))
                @if ($item['input'] == 'rupiah')
